...
struct point
{
    ...
    // to-left 测试，判断 a 是不是在 this 的左边，是返回 1, 右边返回 -1, 相同方向返回 0
...
// 凸多边形
template <typename T>
struct convex : polygon<T>  //凸包继承自多边形结构
{
    // 闵可夫斯基和
...
void solve() {
    ...
    for (int i = 0; i < bcnt; ++i) {
        point_t x, y;
        cin >> x >> y;
        B.p.push_back(Point{x,y});
    }

    auto len = sqrtl(B.rotcaliper());
    cout << setprecision(10) <<  A.circ() + 2 * len * PI << '\n';
